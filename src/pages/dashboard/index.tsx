...
const Dashboard = () => {
  ...
  const handleScroll = (action?: string) => {
    ...
    if (action === "prev") {
      nextIndex =
        currentIndex === 0 ? sectionList.length - 1 : currentIndex - 1;
    } else {
  ...
  const nextSectionName =
    sectionList[(currentIndex + 1) % sectionList.length] || "Home";
  const prevSectionName =
    sectionList[currentIndex === 0 ? sectionList.length - 1 : currentIndex - 1];
    ...
    >
      ...
      <div className="relative w-full h-full">
        ...
        >
          ...
          <span
            className={`absolute right-12 px-4 py-1 rounded-full hidden group-hover:block animate-slide_left2 bg-Primary text-white font-normal text-sm whitespace-nowrap transition ease-in-out !duration-250`}
          >
            {nextSectionName}
          </span>
        </div>
